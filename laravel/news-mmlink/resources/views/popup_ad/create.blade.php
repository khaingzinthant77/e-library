...
@section('content')
    <h3 style="margin-left: 15px">Popup Ads Create</h3>
    <form class="form-horizontal" enctype="multipart/form-data" role="form" method="POST" action="{{ route('popup_ads.store') }}">
      {!! csrf_field() !!}
      <div class="form-group{{ $errors->has('half_or_full') ? ' has-error' : '' }}">
          ...
          <div class="form-group{{ $errors->has('popup_photo') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="file" class="form-control" name="popup_photo">@if ($errors->has('popup_photo'))
                            <span class="help-block">
                                <strong>{{ $errors->first('popup_photo') }}</strong>
                            </span>
                        @endif
           ...
           <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                             <textarea class="form-control" id="summernote" style="height:300px" name="description">{{ old('description') }}</textarea>

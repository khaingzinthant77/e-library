						...
						<div class="box-body">
							<div class="form-group <?=form_error('name') ? 'has-error' : ''?>">
							 	<label for="name"><?=$this->lang->line('expense_name')?></label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id="name" name="name" value="<?=set_value('name', $expense->name)?>" placeholder="Enter Name">
							  	<?=form_error('name')?>
							</div>
							<div class="form-group <?=form_error('date') ? 'has-error' : ''?>">
								<label for="date"><?=$this->lang->line('expense_date')?></label> <span class="text-red">*</span>
							  	<input type="text" class="form-control datepicker" id="date" name="date" value="<?=set_value('date', date('d-m-Y', strtotime($expense->date)))?>" placeholder="Enter Date">
							  	<?=form_error('date')?>
							</div>
							<div class="form-group <?=form_error('amount') ? 'has-error' : ''?>">
							  	...
							  	<input type="text" class="form-control" id="amount" name="amount" value="<?=set_value('amount', $expense->amount)?>" placeholder="Enter Amount">
							  	<?=form_error('amount')?>
							</div>
							...
							<div class="form-group <?=form_error('note') ? 'has-error' : ''?>">
							  	...
							  	<textarea name="note" id="note" class="form-control" cols="30" rows="5"><?=set_value('note', $expense->note)?></textarea>
							  	<?=form_error('note')?>
							</div>
						</div>
						<div class="box-footer">
							<button type="submit" class="btn btn-mytheme"><?=$this->lang->line('expense_update_expense')?></button>
						</div>
					</form>
				</div>
			</div>
